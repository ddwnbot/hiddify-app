                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"VPorN"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"VPorNMutex");
  HWND handle = FindWindowA(NULL, "VPorN");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"VPorN"))
    {
      return false;
    }
  ...
  if (!window.Create(L"VPorN", origin, size))
  {
    return EXIT_FAILURE;
  }
